...
class FordFulkerson{
    ...
    void initialize(vector<vector<int> > graph, int source,int sink,int count){
        ...
        maxFlow = 0;
        // cout<< nodesCount;
        // for(int i=0;i<nodesCount;i++)for(int j=0;j<nodesCount;j++)cout<<recidualGraph[i][j]<<" "<<rough[i]<<"a";
    }
    ...
    void countMaxFlow(){
        ...
        while(bfs()){
            ...
            int prevNode;
            for(int currentNode = sinkNode; currentNode != sourceNode; currentNode = parentNode[currentNode]){
                prevNode = parentNode[currentNode];
                currentPathFlow = min(currentPathFlow,recidualGraph[prevNode][currentNode]);
